...
def add_convolution_params(params, const_params, config, zero_mean_channels=False):
    ...
    if 'dtype' in config and config['dtype'] == 'complex':
        k_0 = generate_random_numbers(config) + 1j * generate_random_numbers(config)
        dtype = tf.complex64
    elif 'dtype' not in config or config['dtype'] == 'float':
        k_0 = generate_random_numbers(config, prox_zero_mean)
        dtype = tf.float32
    else:
        raise ValueError("'dtype' has to match 'float' or 'complex'")

    # filter kernels
    k = tf.Variable(initial_value=k_0, dtype=dtype, name=config['name'])

    if not zero_mean_channels:
        axis = (1, 2, 3)
    else:
        axis = (1, 2)

    mask = np.ones((1, config['filter_size'], config['filter_size'], 1, 1), dtype=np.float32)

    if not zero_mean_channels:
        mask = np.tile(mask, (1, 1, 1, config['features_in'], 1))

    mask_tf = tf.constant(mask, dtype=tf.float32)

    prox_k = proxmaps.zero_mean_norm_ball(k, zero_mean=prox_zero_mean, normalize=prox_norm, axis=axis, mask=mask_tf)
    ...
    with tf.variable_scope('kernel_%s_summary' % config['name']):
        for i in range(config['num_stages']):
            tf.summary.image('%s_%d' % (config['name'], i + 1),
                             get_kernel_img(k[i]), collections=['images'])
